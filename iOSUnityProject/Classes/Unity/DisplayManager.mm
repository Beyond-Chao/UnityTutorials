...
- (void)recreateSurface:(RenderingSurfaceParams)params
{
    ...
    if (UnitySelectedRenderingAPI() == apiMetal)
        _screenSize = CGSizeMake(fminf(screenSize.width, params.renderW), fminf(screenSize.height, params.renderH)); // Enforce native as maximum, otherwise it might run out of memory vert fast
    else
